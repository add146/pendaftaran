...
events.put('/:id', async (c) => {
    const { id } = c.req.param()
    const body = await c.req.json()
    const { title, description, event_date, event_time, location, capacity, event_mode, visibility, status } = body
    ...
    WHERE id = ?
  `).bind(title, description, event_date, event_time, location, capacity, event_mode, visibility, status, id).run()
