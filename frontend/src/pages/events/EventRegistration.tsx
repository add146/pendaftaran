                        ...
                        <div className="lg:col-span-3 space-y-6">
                                ...
                                {images.length === 0 ? (
                                    ...
                                    <>
                                        ...
                                        <div className="absolute bottom-0 left-0 p-6 z-20 pointer-events-none">
                                            <span className={`inline-block px-3 py-1 text-white text-xs font-bold uppercase tracking-wider rounded-full mb-3 mr-2 ${event?.event_mode === 'paid' ? 'bg-amber-500' : 'bg-green-500'}`}>
                                                {event?.event_mode === 'paid' ? t('event.paid_event') : t('event.free_event')}
                                            </span>
                                            {event?.event_type && (
                                                <span className="inline-block px-3 py-1 text-white text-xs font-bold uppercase tracking-wider rounded-full mb-3 bg-blue-500">
                                                    {event.event_type}
                                                </span>
                                            )}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-gray-100 shadow-sm">
                                    <div className="size-10 shrink-0 rounded-full bg-primary/10 text-primary flex items-center justify-center">
                                        <span className="material-symbols-outlined">calendar_month</span>
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <p className="text-xs font-medium text-gray-500 uppercase">{t('event.date_time')}</p>
                                        <p className="font-semibold break-words">{event?.event_date ? formatDate(event.event_date) : '-'}</p>
                                        <p className="text-sm text-gray-500 break-words">{event?.event_time || t('event.time_tba')}</p>
                                    </div>
                                </div>
                                <div className="flex items-start gap-4 p-4 rounded-xl bg-white border border-gray-100 shadow-sm">
                                    <div className="size-10 shrink-0 rounded-full bg-primary/10 text-primary flex items-center justify-center">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="font-semibold break-words">{event?.location || t('event.location_tba')}</p>
                                        {event?.location_map_url && (
                                            ...
                                            >
                                                <span className="material-symbols-outlined text-[14px] shrink-0">map</span>
